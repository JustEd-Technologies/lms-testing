<script lang="ts">
  ...
  import CircleContent from '$lib/pages/Quiz/PlayHeader/CircleContent/index.svelte';
  ...
  function triggerCountDown(start: boolean) {
    if (!start || started) return;

    const getNum = (k: string) => parseInt(timelimit.replace(k, ''));
