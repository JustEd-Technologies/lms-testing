<script lang="ts">
  ...
  import CloseButton from '$lib/ui/components/Button/CloseButton/index.svelte';
  ...
  >
    ...
    <div
      class="{maxWidth || 'container'} {size === 'sm'
        ? 'small'
        : ''} mx-auto bg-white  {width} rounded-md pt-2 shadow-lg"
      on:click={(e) => e.stopPropagation()}
      role="presentation"
    >
      <div
        class="flex items-center justify-between border border-l-0 border-r-0 border-t-0 border-gray-100  p-4 px-5 {headerClass}"
      >
        <p class="text-md m-0 font-medium  {labelClass}">
          {modalHeading}
        </p>
        {#if isCloseable}
